...
int main(void){
  ...
  reqs requirements_classes[NUMBER_OF_CLASSES];
  ...
  for(i = 0; i < NUMBER_OF_CLASSES; i++){
    printf("  The fitness is: %d   8.A's Skoleschedule: \n", chosen_individual[i][49].fitness);
    create_schedule(week, chosen_individual[i][49], i);
    print_schedule(week, i);
  }

  for(j = 0; j <NUMBER_OF_CLASSES; j++){
    for(i = 0; i < NUMBER_OF_FAG; i ++){
      printf("%d\n", teacher_data[j][i].number_of_lessons);
    }
    printf("\n\n");
  }
  return 0;
